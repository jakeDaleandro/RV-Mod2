...
char **argv;
{
    int i, j, p, q, s, t, x, y, mr, centx, centy;
    double maskval, xmask, ymask, sum, sig, maxival, minival, maxval, ZEROTOL;
    ...
    for (x = -mr; x <= mr; x++)
    {
        for (y = -mr; y <= mr; y++)
        {
            xmask = x*exp(-1 * (((x * x) + (y * y)) / (2 * (sig * sig))));
            ymask = y*exp(-1 * (((x * x) + (y * y)) / (2 * (sig * sig))));
            (mask[x + centy][y + centx]) = maskval;
        }
    }
